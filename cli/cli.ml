(** Caravan's command line: REPL, one-shot agent runs, and utilities. *)
...
(** Parse the command line and run.  The executable in [bin/] is a
    one-liner over this, so every module here stays reachable from the
    test suite. *)
let main () =
